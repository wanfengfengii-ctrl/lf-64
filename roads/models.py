...
class OpenSchedule(models.Model):
    ...
    season = models.CharField('适用季节', max_length=20, choices=SEASON_CHOICES)
    ...
    time_slot_minutes = models.IntegerField('分时段时长(分钟)', default=60)
    slot_max_capacity = models.IntegerField('分时段最大人数', default=0, help_text='0表示不设分时限流')
    ...
    def clean(self):
        if self.max_capacity <= 0:
            raise ValidationError({'max_capacity': '最大承载人数必须大于 0。'})
        if self.time_slot_minutes <= 0:
            raise ValidationError({'time_slot_minutes': '分时段时长必须大于 0。'})
        if self.slot_max_capacity < 0:
            raise ValidationError({'slot_max_capacity': '分时段最大人数不能小于 0。'})
        if self.slot_max_capacity and self.slot_max_capacity > self.max_capacity:
            raise ValidationError({'slot_max_capacity': '分时段最大人数不能超过最大承载人数。'})
        if self.open_time == self.close_time:
            raise ValidationError({'close_time': '开放结束时间不能与开始时间相同。'})
...
class TemporaryControl(models.Model):
    ...
    control_type = models.CharField('管制类型', max_length=30, choices=CONTROL_TYPE_CHOICES)
    ...
    end_time = models.DateTimeField('预计结束时间', null=True, blank=True)
    adjusted_capacity = models.IntegerField('调整后承载人数', null=True, blank=True, help_text='留空表示按原计划')
    adjusted_open_time = models.TimeField('调整后开放时间', null=True, blank=True)
    adjusted_close_time = models.TimeField('调整后关闭时间', null=True, blank=True)
    ...
    def __str__(self):
        return f'{self.road_section.code} - {self.get_control_type_display()}'

    def clean(self):
        if self.end_time and self.end_time <= self.start_time:
            raise ValidationError({'end_time': '预计结束时间必须晚于开始时间。'})
        if self.adjusted_capacity is not None and self.adjusted_capacity < 0:
            raise ValidationError({'adjusted_capacity': '调整后承载人数不能小于 0。'})
...
class VisitorFlowRecord(models.Model):
    ...
    current_occupancy = models.IntegerField('当前在段人数', default=0)
    ...
    def __str__(self):
        return f'{self.road_section.code} - {self.record_date} {self.record_time}'

    def clean(self):
        if self.visitor_count < 0:
            raise ValidationError({'visitor_count': '游客数量不能小于 0。'})
        if self.current_occupancy < 0:
            raise ValidationError({'current_occupancy': '当前在段人数不能小于 0。'})
